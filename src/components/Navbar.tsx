...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="hidden md:flex fixed top-6 left-0 right-0 z-50 justify-center pt-0 pb-6 font-consolas">
        <div
        ...
        >
          {menu.map((item) => (
            <a
              ...
              className={`text-sm tracking-wide transition-colors duration-200 ${activeSection === item.href.replace('#', '')
        ...
        <div className="flex justify-around py-3">
          {menuMobile.map((item) => (
            <a
              ...
              href={item.href}
              onClick={e => handleNavClick(e, item.href)}
              className={`flex flex-col items-center text-xs ${activeSection === item.href.replace('#', '')
